  ...
  return (
    <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className={`overflow-hidden transition-all duration-500 ${filtersOpen ? "max-h-[800px] opacity-100 mt-4" : "max-h-0 opacity-0"}`}>
          <div className="bg-teal-50/30 rounded-[2rem] p-6 border border-teal-100/50 shadow-inner grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <FilterSelect label="Brand" value={currentParams.brand} options={brands} onChange={(v) => updateFilter("brand", v)} />
            <FilterSelect label="Category" value={currentParams.category} options={categories} onChange={(v) => updateFilter("category", v)} />
            <FilterSelect label="Model Family" value={currentParams.model_family} options={model_family} onChange={(v) => updateFilter("model_family", v)} />
            <FilterSelect label="Condition" value={currentParams.condition} options={conditions} onChange={(v) => updateFilter("condition", v)} />
            <FilterSelect label="Main Category" value={currentParams.category_main} options={category_main} onChange={(v) => updateFilter("category_main", v)} />
          </div>
        </div>
      </div>
    </div>
